  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-950 via-brand-950 to-gray-950 flex flex-col items-center justify-center px-4 py-10">
    ...
    <div class="w-full max-w-sm floating-card animate-slide-up">
      ...
      <div class="mt-6 border-t border-gray-800 pt-4">
        <p class="text-xs text-gray-500 mb-2 text-center">Contas de demonstração:</p>
        <div class="space-y-1">
          <button
            v-for="acc in demoAccounts" :key="acc.email"
            class="w-full text-left text-xs text-gray-400 hover:text-brand-400 transition-colors px-2 py-1 rounded hover:bg-gray-800"
            @click="fillDemo(acc)"
          >
            <span class="text-brand-400">{{ acc.role }}</span> — {{ acc.email }}
          </button>
        </div>
      </div>
    </div>
...
const demoAccounts = [
  { email: 'admin@agrovet.com', password: 'admin123', role: 'Admin' },
  { email: 'vet@agrovet.com',   password: 'vet123',   role: 'Veterinário' },
  { email: 'op@agrovet.com',    password: 'op123',    role: 'Operador' },
]
